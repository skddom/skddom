...
//            $action = "message";
    ...
    public function output_page_buffer() {
        ...
        if (!$this->inside_admin) {
            ...
            // Временное решение, нужно или перенести полностью в модуль stats
            // или добавить событие «готовность страницы»
            if (nc_module_check_by_keyword('stats') && nc_stats::get_instance()->should_add_analytics_scripts()) {
                $buffer = nc_stats::get_instance()->analytics->process_page_buffer($buffer);
            }
        }
        ...
        if (function_exists("custom_modify_output")) custom_modify_output($buffer);
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/d-seoMeta.php')) {
            include_once($_SERVER['DOCUMENT_ROOT'] . '/d-seoMeta.php');
        }
        	$buffer = str_replace("http://www.skd","https://www.skd",$buffer);
        	$buffer = str_replace("http://skd","https://skd",$buffer);
            echo $buffer;
        }
    }
