...
abstract class nc_security_filter {
    ...
    /** @var string[] регулярные выражения или строка с одним символом — для пре-фильтрации входящих данных */
    ...
    protected function examine_input_recursive(array $input, $parent_source = null) {
        foreach ($input as $key => $value) {
            if ($parent_source === '$_GET' && $key === 'REQUEST_URI') {
                continue;
            }
            ...
            if (is_array($value)) {
                ...
                foreach (static::$patterns as $name => $pattern) {
                    if (strlen($pattern) === 1) {
                        $is_suspicious = (strpos($value, $pattern) !== false);
                    } else {
                        $is_suspicious = (preg_match($pattern, $value) === 1);
                    }

                    if ($is_suspicious) {
                        $this->suspicious_input[$name][$value_source] = $value;
                    }
                }
            }
        }
    ...
    public function filter($checked_string, $context = null) {
        ...
        if (!$this->is_next_check_skipped) {
            ...
            $result = $this->check_string_against_input($checked_string, $input, $context);
        }
    ...
    protected function trigger_error($check_type, $checked_string, $input_source, $input_value) {
        ...
        switch ($this->mode) {
            ...
            case self::MODE_RELOAD_ESCAPE_INPUT:
                $escaped_input_value = $this->escape_input($input_value, $check_type);
     ...
     *
     * @param string $input строка, вызвавшая срабатывание фильтра
     * @param string $check_type тип проверки (контекст), вызвавший необходимость экранирования
     * @return string
     */
    abstract protected function escape_input($input, $check_type);
    ...
    protected function reload($input_substitute) {
        ...
        } else if ($url !== $nc_core->url->get_local_url()) {
            header("Location: $url");
        } else {
