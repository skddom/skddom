...
class Filemanager {
  ...
  public function __construct($config) {
    ...
    if(isset($this->config['doc_root'])) $this->doc_root = $this->config['doc_root'];
      ...
      $this->doc_root = $DOCUMENT_ROOT.$SUB_FOLDER;
    }
    $this->setParams();
  ...
  public function add() {
    ...
    if(!isset($_FILES['newfile']) || !is_uploaded_file($_FILES['newfile']['tmp_name'])) {
      $this->error(sprintf($this->lang('NO_FILE')),true);
    }
    ...
    $_FILES['newfile']['name'] = $this->cleanString($_FILES['newfile']['name'], array('.','-',' ','_'));
  ...
  public function addfolder() {
  	if (!preg_match("/^[{$allow}a-zA-Z0-9]/", $this->get['name']))
        $this->get['name'] = nc_transliterate($this->get['name']);
