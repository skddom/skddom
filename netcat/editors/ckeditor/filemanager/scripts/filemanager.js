...
//document.write('<scri'+'pt type="text/javascript" src="scripts/languages/ru_cp1251.js"><\scri'+'pt>');
...
var selectItem = function(data){
	if(window.opener){
		...
		if($.urlParam('CKEditor')){
			...
			var  a =  $.ajax( { 
			  url: fileConnector + '?mode=getbasedir',
			  async : false } ).responseText;
				window.opener.CKEDITOR.tools.callFunction($.urlParam('CKEditorFuncNum'), a+data['Path']);
			} else {
...
var addNode = function(path, name){
	...
	if(parentNode.attr('id')=='') parentNode.find('ul').prepend(newNode); 
	else $('#filetree').find('ul').prepend(newNode);
	if(!parentNode.find('ul').size()) parentNode.append('<ul></ul>');	

	thisNode.click().click();
	$('#filetree').find('li a[rel="' + path + name + '"]').unbind("click");
		$('#filetree').find('li a[rel="' + path + name + '"]').click(function(){ 
...
//bindEvent(window, 'load', function() {getDetailView('/1/');});
