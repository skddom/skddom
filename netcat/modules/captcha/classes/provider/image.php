...
class nc_captcha_provider_image extends nc_captcha_provider {
    ...
    public function generate_challenge($challenge_id = null) {
        ...
        $alphabet = array_values(array_filter(nc_preg_split('//', $alphabet), 'strlen')); // массив с символами (возможно, utf8)
        ...
        $db->query(
            "REPLACE INTO `Captchas` (`Captcha_Hash`, `Captcha_Code`)
             VALUES ('" . $db->escape($challenge_id) . "', '" . $db->escape($captcha_code) . "')"
        );
    ...
    protected function update_audio_captcha_files() {
        if (!$this->get_setting('Image_Audio')) {
            return;
        }
        ...
        $will_update_settings =
            ...
            is_dir($nc_core->MODULE_FOLDER . 'captcha/voice/' . $voice . '/') &&
        ...
        $from = $nc_core->MODULE_FOLDER . 'captcha/voice/' . $voice . '/';
        ...
        foreach ($src_mp3_files as $src_file) {
            ...
            if ($captcha_settings['current_voice'] != $voice || !in_array($captcha_settings[$src_file], $enc_mp3_files)) {
    ...
    protected function set_audio_captcha_setting($key, $value) {
        $db = nc_db();
        $set = "SET `Value`= '" . $db->escape($value) . "' WHERE `Key` = '" . $db->escape($key) . "'";
        $db->query("UPDATE `Captchas_Settings` $set");

        if (!$db->rows_affected) {
            $db->query("INSERT INTO `Captchas_Settings` $set");
        }
    }
    ...
    public function get_challenge_html($challenge_id = null, array $parameters = array()) {
        ...
        $attributes = nc_array_value($parameters, 'img_attributes', '');
        ...
        $result =
            (!$refresh && !$no_refresh_button ? "<div id='nc_captcha_container" . $id . "' style='display:inline-block'>" : "") . "
            <input type='hidden' name='" . $this->get_hash_field_name() . "' value='" . $captcha_hash . "' />
            <img name='nc_captcha_img' src='" . nc_module_path('captcha') . "img.php?code=" . $captcha_hash . "' " . $attributes . "/>" .
            $this->get_audio_captcha_html($captcha_hash, $id, $parameters) .
            (!$refresh && !$no_refresh_button ? "</div>
            <button id='nc_captcha_refresh_button" . $id . "' type='button'>" . $btn . "</button>
            " . $this->get_captcha_js('nc_captcha_container' . $id, 'nc_captcha_refresh_button' . $id, 'nc_captcha' . $id) : "");
    ...
    protected function get_audio_captcha_html($captcha_hash, $id, $parameters) {
        ...
        $audio_captcha_player = '';

        $add_audio_captcha =
            $this->get_setting('Image_Audio') &&
            is_writable($nc_core->FILES_FOLDER) &&
            is_dir($nc_core->MODULE_FOLDER . 'captcha/voice/' . $this->get_setting('Image_Voice') . '/');

        if ($add_audio_captcha) {
            $module_path = nc_module_path('captcha');

            $button_prefix = nc_array_value($parameters, 'audio_prefix') ?:
                "<span class='nc_captcha_voice' style='margin:0 0 0 10px;'>" .
            ...
            $button_suffix = nc_array_value($parameters, 'audio_suffix') ?: "</a><span id='nc_captcha_player' style='display:none;'></span></span>";

            $audio_captcha = $button_prefix . $button_content . $button_suffix;

            $player = "<script type='text/javascript' src='" . $module_path . "player/swfobject.js'></script>" .
                      "<script type='text/javascript' src='" . $module_path . "player/uppod_player.js'></script>" .
                      "<script type='text/javascript' src='" . $module_path . "js/nc_audiocaptcha.js'></script>";
            $playlist = "{'playlist':[";
            $captcha_code = $nc_core->db->get_var("SELECT `Captcha_Code` FROM `Captchas` WHERE `Captcha_Hash` = '" . $captcha_hash . "'");
            $code_hash = $nc_core->db->get_results("SELECT * FROM `Captchas_Settings`") ?: array();
                ...
                foreach ($letters as $letter) {
                    ...
                    foreach ($code_hash as $hash) {
                        if ($hash->Key == $letter . '.mp3') {
                            $playlist .= "{'file':'" . $nc_core->HTTP_FILES_PATH . "captcha/current_voice/" . $hash->Value . "'},";
                            break;
                        }
                    }
                }
            ...
            $audio_captcha_player = $player . $player_init . $audio_captcha;
        }

        return $audio_captcha_player;
    }
            ...
            jQuery('#" . $id_refresh_button . "').click(function() {
                jQuery.ajax({url: '" . $module_path . "index.php?nc_get_new_captcha=1',
                        success: function(result) {
                            ...
                            jQuery('#" . $id_captcha_container . " input[name=" . $this->get_hash_field_name() . "]').attr('value', res[0]);
                            if (res[1] > '') {
    ...
    public function delete_expired_challenges() {
        $captcha_expiration = (int)$this->get_setting('Image_ExpiresIn') ?: self::DEFAULT_EXPIRES_IN; // время жизни каптчи
        nc_db()->query("DELETE FROM `Captchas` WHERE `Captcha_Created` <= DATE_SUB(NOW(), INTERVAL $captcha_expiration SECOND)");
    }
    ...
    public function verify_user_response($user_response, $challenge_id = null, $invalidate_challenge = true) {
        ...
        if (!$challenge_id) {
            $challenge_id = $nc_core->input->fetch_get_post($hidden_field_name);
        }
        ...
        $response_is_correct = (bool)$db->get_var(
            "SELECT 1
              ...
              WHERE `Captcha_Hash` = '$escaped_challenge_id'
                AND UPPER(`Captcha_Code`) = UPPER('" . $db->escape($user_response) . "')
