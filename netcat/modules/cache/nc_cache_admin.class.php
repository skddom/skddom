...
/* $Id: nc_cache_admin.class.php 8444 2012-11-20 14:36:28Z vadim $ */
...
class nc_cache_admin {
    ...
    public function settings() {
        ...
        if (!$Catalogue_ID) $Catalogue_ID = $catalogues[0]['Catalogue_ID'];
    ...
    public function settingsSave() {
        ...
        // simple check
        if (!( is_numeric($Catalogue_ID) && $Catalogue_ID > 0 )) return false;
        ...
        if (!empty($_fields) && !empty($this->POST)) {
            ...
            foreach ($this->POST AS $key => $value) {
                // only need fields
                if (!in_array($key, $_fields) || $key == 'ID') continue;
                // append to query array
                $query_arr[] = "`".$key."` = '".$this->db->escape($value)."'";
            }
            if (empty($query_arr)) return false;
            // existence
            ...
            // save settings
            if ($SettingsExist) {
                $this->db->query("UPDATE `Cache_Settings` SET ".join(", ", $query_arr)." WHERE `Catalogue_ID` = '".$Catalogue_ID."'");
            } else {
                $this->db->query("INSERT INTO `Cache_Settings` SET ".join(", ", $query_arr));
            }
    ...
    public function info() {
        ...
        require_once 'modules/nc_cache_calendar.class.php';
    ...
    public function auditInfo() {
        ...
        if (!$cache_sortby || !in_array($cache_sortby, array("desc", "asc")))
                $cache_sortby = "desc";
