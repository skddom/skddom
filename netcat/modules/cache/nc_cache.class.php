...
abstract class nc_cache {
     ...
     *
     * @param int content lenght size
     * @return bool true if quota not expired or false
     */
    protected function checkQuota($lenght) {
        if ($this->io instanceof nc_cache_io_memcache) return true;
        // content quota from settings
        $content_quota = $this->settings["Quota_".$this->essence] * 1024 * 1024;
        // get stats for this cache dir
        $content_lenght = $this->getStat();
        // check size
        if ($content_quota >= ($content_lenght + $lenght) || $content_quota == 0)
                return true;
        // overdraft value for this essence
        switch ($this->settings["Overdraft_".$this->essence]) {
            // don't write new cache
            case false:
            case 1:
                $this->debugMessage("Quota limit ".$content_quota.", cache size ".$content_lenght.", data to write ".$lenght, __FILE__, __LINE__, "error");
                return false;
            // try to clear low efficiency cache and write new
            case 2:
                $total_bytes = $this->dropEfficiencyCache(1);
                // get stats for this cache dir once more
                $new_content_lenght = $this->getStat();
                // check size once more
                if ($content_quota >= ($new_content_lenght + $lenght) || $content_quota == 0) {
                    $this->debugMessage("Low efficiency cache ".$total_bytes." cleared from disk. Quota limit ".$content_quota.", cache size ".$new_content_lenght.", data to write ".$lenght, __FILE__, __LINE__, "info");
                    return true;
                }
                $this->debugMessage("Low efficiency cache ".$total_bytes." cleared from disk. Quota limit ".$content_quota.", cache size ".$new_content_lenght.", data to write ".$lenght, __FILE__, __LINE__, "error");
                return false;
        }
    }
