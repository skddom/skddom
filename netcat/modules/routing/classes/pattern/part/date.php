...
//            $parameters->date = $result;
    ...
    protected function folder_has_component_with_event_field($folder_id) {
        $nc_core = nc_core::get_object();
        foreach ((array)$nc_core->sub_class->get_by_subdivision_id($folder_id) as $infoblock_settings) {
            if ($nc_core->get_component($infoblock_settings['Class_ID'])->get_date_field()) {
                return true;
            }
        }
        return false;
