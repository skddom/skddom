...
class nc_routing_pattern_part_object_action extends nc_routing_pattern_part {

    public function match(nc_routing_request $request, nc_routing_result $result) {
        if (nc_preg_match('/^(edit|delete|drop|checked|subscribe)/', $result->get_remainder(), $matches)) {
            $action = $matches[1];
