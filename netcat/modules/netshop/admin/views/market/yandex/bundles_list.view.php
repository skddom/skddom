...
?>
<div class="nc_admin_fieldset_head"><?= NETCAT_MODULE_NETSHOP_YANDEX_MARKET_BUNDLES; ?></div>
<?php
...
$thead->th(NETCAT_MODULE_NETSHOP_YANDEX_MARKET_BUNDLE_ID)->compact()->text_center();
$thead->th(NETCAT_MODULE_NETSHOP_YANDEX_MARKET_BUNDLES_NAME);
$thead->th(NETCAT_MODULE_NETSHOP_YANDEX_MARKET_BUNDLES_EXPORT_URL)->text_center()->wide();
$thead->th(NETCAT_MODULE_NETSHOP_YANDEX_MARKET_BUNDLES_UPDATED)->compact();
$thead->th(NETCAT_MODULE_NETSHOP_YANDEX_MARKET_BUNDLES_EDIT_FIELDS)->compact();
$thead->th()->compact();
...
foreach ($bundles as $row) {
    ...
    $tr->export_url->text(nc_get_scheme() . '://' . $domain . nc_module_path('netshop') . 'export/yandex/bundle' . $bundle_id . '.xml');
    ...
    $tr->delete_button->text($ui->controls->delete_button(
        sprintf(NETCAT_MODULE_NETSHOP_YANDEX_CONFIRM_DELETE, $bundle->get('name')),
        $post_actions_params
    ));
