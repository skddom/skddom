...
?>
<div class="nc_admin_fieldset_head"><?= NETCAT_MODULE_NETSHOP_YANDEX_MARKET_BUNDLES_EDIT_FIELDS . " '" . $bundle->get('name') . "'" ?></div>
<?php
  ...
  (function() {
      ...
      nc_market_multifields.prototype = {
        add: function(link_obj, goods_table, field_name, name, units, field, selected_val) {
          ...
          $nc('#' + con_id).append("<div class='name'><?php echo NETCAT_MODULE_NETSHOP_YANDEX_MARKET_MULTI_NAME ?><br /><input name='map_fields["+goods_table+"]["+field_name+"][name][]' type='text' value='" + name + "' /></div>");
          $nc('#' + con_id).append("<div class='field'><?php echo NETCAT_MODULE_NETSHOP_YANDEX_MARKET_MULTI_FIELD ?><br /><select name='map_fields["+goods_table+"]["+field_name+"][field][]'>" + field + "</select></div>");
          $nc('#' + con_id).append("<div class='units'><?php echo NETCAT_MODULE_NETSHOP_YANDEX_MARKET_MULTI_UNITS ?><br /><input name='map_fields["+goods_table+"]["+field_name+"][units][]' type='text' value='" + units + "' /></div>");
