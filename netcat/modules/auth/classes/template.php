...
class nc_auth_template {
    ...
    public function get_vk() {
        global $nc_auth, $nc_auth_vk, $REQUEST_URI;
        return "
...
<script type='text/javascript'>
  function nc_vk_login ( is_member ) {
    ...
    else location.href='" . $this->core->SUB_FOLDER . $this->core->HTTP_ROOT_PATH . "modules/auth/?nc_vk=1&REQUESTED_FROM=" . htmlspecialchars($REQUEST_URI, ENT_QUOTES) . "';
  }
...
<script type='text/javascript'>
    window.fbAsyncInit = function() {
        ...
        function updateButton(response) {
            if (response.authResponse) {
                location.href = '".$this->core->SUB_FOLDER.$this->core->HTTP_ROOT_PATH . "modules/auth/?nc_fb=1&token='+response.authResponse.accessToken+'&REQUESTED_FROM=".htmlspecialchars($REQUEST_URI, ENT_QUOTES)."';
            } else {
    ...
    public function get_twitter() {
        return "<div class='twitter'><a href='" . $this->core->SUB_FOLDER . $this->core->HTTP_ROOT_PATH . "modules/auth/?nc_twitter=1'><img src='". $this->core->SUB_FOLDER . $this->core->HTTP_ROOT_PATH . "modules/auth/images/icons/twitter.png' alt='twitter' /></a></div>";
    }

    public function get_openid() {
        $providers = unserialize($this->core->get_settings('ex_openid_providers', 'auth'));
        $result = "<form action='{$this->core->SUB_FOLDER}{$this->core->HTTP_ROOT_PATH}modules/auth/'> <div class='nc_auth_openid'><div class='auth_header'>OpenID</div>";
        if (!empty($providers)) {
	...
	public function get_oauth() {
		global $nc_auth, $nc_auth_oauth;
		$providers = unserialize($this->core->get_settings('ex_oauth_providers', 'auth'));
		$result = "<form action='" . $this->core->SUB_FOLDER . $this->core->HTTP_ROOT_PATH . "modules/auth/'> <div class='nc_auth_oauth'><div class='auth_header'>OAuth</div>";
		if (!empty($providers)) {
            foreach ($providers as $pr) {
