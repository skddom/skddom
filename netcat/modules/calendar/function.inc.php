...
function nc_set_calendar($id = 0) {
    ...
    $result .= "<script type='text/javascript' src='" . $ADMIN_PATH . "js/lib.js'></script>\n";
    $result .= "<script type='text/javascript' src='" . $SUB_FOLDER . $HTTP_ROOT_PATH . "modules/calendar/calendar.js'></script>\n";
...
function nc_show_calendar($theme = 0, $cc = 0, $setDate = 0, $DateField = "Date", $filled = false, $queryDate = false, $popup = false, $calendar_fields = array(), $cc_ignore = false, $admin_mode = 0) {
    ...
    if ($popup) {
        ...
        $CalendarHeader = str_replace("%LINK_PREV_MONTH", ($IMG_PREV_MONTH ? "<a href='#' onclick='nc_calendar_generate(" . ($day ? $day : 1) . ", " . ($month == 1 ? 12 : $month - 1) . ", " . ($month == 1 ? $year - 1 : $year) . " " . ($cc_ignore ? ", true " : ", false ") . ", ".($admin_mode ? 1 : 0)."); return false;' alt='" . $MonthArray[$month == 1 ? 12 : $month - 1] . "' title='" . $MonthArray[$month == 1 ? 12 : $month - 1] . "' ></a>" : "&lt;"), $CalendarHeader);
        $CalendarHeader = str_replace("%LINK_NEXT_MONTH", ($IMG_NEXT_MONTH ? "<a href='#' onclick='nc_calendar_generate(" . ($day ? $day : 1) . ", " . ($month == 12 ? 1 : $month + 1) . ", " . ($month == 12 ? $year + 1 : $year) . " " . ($cc_ignore ? ", true " : ", false ") . ", ".($admin_mode ? 1 : 0)."); return false;' alt='" . $MonthArray[$month == 12 ? 1 : $month + 1] . "' title='" . $MonthArray[$month == 12 ? 1 : $month + 1] . "' ></a>" : "&gt;"), $CalendarHeader);
    }
    $DateFieldExist = true;
    $path = $nc_core->SUB_FOLDER . $nc_core->HTTP_ROOT_PATH . '?sub=' . $_sub_id . '&amp;cc=' . $_cc_id;
