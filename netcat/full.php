...
do {
    ...
    foreach ($text_fields as $field) {
        ...
        if ($format['bbcode']) {
            if ($html_disabled) {
                $cc_env['convert2txt'] .= "\$f_" . $field['name'] . " = nc_bbcode(\$f_" . $field['name'] . ",  '', true);";
            } else {
                $cc_env['convert2txt'] .= "\$f_" . $field['name'] . " = nc_bbcode(htmlspecialchars(\$f_" . $field['name'] . ", ENT_QUOTES), '', true);";
            }
        }
        // перенос строки
    ...
    $nc_core->page->update_last_modified_if_newer($f_Created, 'content');
    $nc_core->page->update_last_modified_if_newer($f_LastUpdated, 'content');
