...
if ($use_multi_sub_class && !$cc_keyword) {
    ...
    else if ($nc_core->admin_mode && !$nc_core->inside_admin) {
        $nc_main_content =
            "<div class='nc-infoblock'>" .
            nc_admin_infoblock_insert_toolbar($sub, 0, 'first');
            "</div>";
    }
...
// Главная страница часто формируется только макетом дизайна (без инфоблоков в разделе главной страницы),
// поэтому на ней будем считать весь контент «основным»
