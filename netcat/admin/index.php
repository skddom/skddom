...
?><!DOCTYPE html>
<!--[if lt IE 7]>
<html style="overflow-y:hidden" class="nc-ie6 nc-oldie"><![endif]-->
<!--[if IE 7]>
<html style="overflow-y:hidden" class="nc-ie7 nc-oldie"><![endif]-->
<!--[if IE 8]>
<html style="overflow-y:hidden" class="nc-ie8 nc-oldie"><![endif]-->
<!--[if gt IE 8]><!-->
<html style="overflow-y:hidden"><!--<![endif]-->
<head>
	...
	if ( ! empty( $modules ) ) {
		...
		foreach ( $modules as $module ) {
			...
			if ( file_exists( $MODULE_FOLDER . $module['Keyword'] . "/nc_" . $module['Keyword'] . "_admin.class.php" ) ) {
				...
				if ( class_exists( $cn = "nc_" . $module['Keyword'] . "_admin" ) && file_exists( $MODULE_FOLDER . $module['Keyword'] . "/function.inc.php" ) ) {
					require_once $MODULE_FOLDER . $module['Keyword'] . "/function.inc.php";
					$admin_obj = new $cn();
...
if ( $perm->isAccess( NC_PERM_REPORT ) ) {
	...
	$navbar->tray->add_btn( '#tools.systemmessages' )->compact()
	             ->title( $ANY_SYSTEM_MESSAGE ? BEGINHTML_ALARMON : BEGINHTML_ALARMOF, TRUE )
...
?>
<script type="text/javascript" src="/netcat/modules/stages/ajax.js?<?php print microtime(true);?>"></script>
</body>
</html>
