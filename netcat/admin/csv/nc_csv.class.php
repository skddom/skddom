...
class nc_csv
{
    ...
    const DEFAULT_MAX_LINE_LENGTH = 10000;
    ...
    protected function export($options) {
        $component_id = (int)nc_array_value($options, 'component_id') ?: $this->get_class_id(nc_array_value($options, 'subclass_id'));
        if (!$component_id) {
        ...
        $result = $db->get_results(
            "SELECT Field_Name, TypeOfData_ID, Format
               FROM Field
              WHERE Class_ID = " . intval($component_id) . "
                AND TypeOfData_ID IN (" . implode(",", $this->get_allowed_field_types($include_list_fields)) . ")
              ORDER BY Priority",
            ARRAY_N);
        ...
        foreach ($result as $result_row) {
            list($field_name, $field_type, $field_format) = $result_row;
            $fields_in_sql[] = "`$field_name`";
            if ($field_type == NC_FIELDTYPE_SELECT) {
                $list_fields[$field_name] = strtok($field_format, ":");
                if ($include_list_ids) {
                    $fields_in_csv[] = "$field_name.ID";
                }
                if ($include_list_names) {
                    $fields_in_csv[] = "$field_name";
                }
                if ($include_list_values) {
                    $fields_in_csv[] = "$field_name.Value";
                }
            }
            else {
                $fields_in_csv[] = $field_name;
    ...
    public function preimport_file($file, $data)
    {
        ...
        $fields = (array)$nc_core = nc_core::get_object()->db->get_col(
    ...
    public function import_file($file, $data)
    {
        ...
        $csv_data = $this->process_csv($file, $data['csv']);
        ...
        if ($include_list_fields) {
            $list_fields_info = (array)$db->get_results(
                "SELECT Field_Name, Format
                   FROM Field
                  WHERE Class_ID = $component_id
                    AND TypeOfData_ID = " . NC_FIELDTYPE_SELECT,
                ARRAY_N);
            foreach ($list_fields_info as $f) {
                $list_fields[$f[0]] = strtok($f[1], ":");
            }
        }
        ...
        return array('success' => $imported_record_count);
...
//        }
    ...
    protected function get_default_line_length() {
        return defined('MAX_LINE_LENGTH') ? MAX_LINE_LENGTH : self::DEFAULT_MAX_LINE_LENGTH;
    }

    protected function process_csv_header($file, $settings)
    {
        $max_line_length = $this->get_default_line_length();
        $nc_core = nc_core::get_object();
