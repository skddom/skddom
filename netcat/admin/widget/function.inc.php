...
/* $Id: function.inc.php 8443 2012-11-20 11:43:28Z vadim $ */
...
function nc_widget_add_form($params = '', $widget_id = '') {
    ...
    "</form>".
    "<script type='text/javascript' src='".$ADMIN_PATH."js/widget.js'></script>".
...
function nc_widgetclass_info($widgetclass_id) {
    ...
    $fields = $nc_core->db->get_results("SELECT `Field_Name`, `DefaultState`, `Description`
                              FROM `Field`
                              WHERE `Widget_Class_ID` = ".$widgetclass_id."", ARRAY_A);
    if ($fields)
            foreach ($fields as $f) {
...
function is_exist_keyword($keyword, $widget_class_id=0, $widget_id=0) {
    global $db;
    $query = ($widget_class_id ? "SELECT * FROM `Widget_Class` WHERE `Keyword`='".$keyword."' AND `Widget_Class_ID`<>'".$widget_class_id."'" : "SELECT * FROM `Widget` WHERE `Keyword`='".$keyword."'AND `Widget_ID`<>'".$widget_id."'");
