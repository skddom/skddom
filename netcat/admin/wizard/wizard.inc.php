...
function nc_site_wizard_map() {
    global $nc_core, $db;
    global $HTTP_DOMAIN, $HTTP_HOST, $DOMAIN_NAME, $EDIT_DOMAIN, $HTTP_ROOT_PATH, $CatalogueID;
...
function nc_site_wizard_map_write_sub($ParentSubID, $CatalogueID, $count = 1) {
    global $db, $nc_core;
    global $DOMAIN_NAME, $HTTP_ROOT_PATH, $EDIT_DOMAIN, $DOMAIN_NAME;
    ...
    if ($Result = $db->get_results($select, ARRAY_N)) {
        ...
        foreach ($Result as $Array) {
            $hidden_host = $nc_core->catalogue->get_scheme_by_id($Array[6]) . '://' . ($Array[5] ? (strchr($Array[5], ".") ? $Array[5] : $Array[5] . "." . $DOMAIN_NAME) : $DOMAIN_NAME);
