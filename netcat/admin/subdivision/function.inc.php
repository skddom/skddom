...
// quotes added into the message_put.php!
        ...
        if (!empty($fl)) {
            $real_value = $nc_core->page->fetch_page_metatags($nc_core->catalogue->get_scheme_by_id($loc->CatalogueID) . '://' . $nc_core->DOMAIN_NAME . nc_folder_path($message));
...
function ShowFullSubdivisionList() {
    ...
    // view
    echo "<td class='nc--compact'><a href=" . nc_get_scheme() . '://' . ($Array[2] ?: $DOMAIN_NAME) . ($SUB_FOLDER ? $SUB_FOLDER . '/' : NULL) . (strlen(session_id()) > 0 ? "?" . session_name() . "=" . session_id() . "" : "") . "' target=_blank><i class='nc-icon nc--arrow-right nc--hovered' title='" . CONTROL_CONTENT_CATALOUGE_FUNCS_SHOWMENU_A_VIEW . "'></i></a></td>";
...
function nc_subdivision_show_edit_form($sub_id, $view) {
    ...
    foreach ($views as $view) {
        $tab_caption_constant = 'SUBDIVISION_TAB_INFO_TOOLBAR_EDIT_' . strtoupper($view);
        echo '<div data-tab-caption="' .htmlspecialchars(constant($tab_caption_constant)) . '" data-tab-id="' . $view . '">';
...
function nc_subdivision_form_seo($sub_env, $have_default = true) {
    ...
    if ($have_default) {
        ...
        $mt = $nc_core->page->fetch_page_metatags($nc_core->catalogue->get_scheme_by_id($sub_env['Catalogue_ID']) . '://' . $domain . nc_folder_path($sub_env['Subdivision_ID']));
        // title
    ...
    if ($have_default) {
        ...
        $f_ncSMO_Image = $nc_core->subdivision->get_by_id($sub_id, 'ncSMO_Image');

        if (!empty($f_ncSMO_Image)) {
            list ($filename, $filetype, $filesize, $filepath) = explode(':', $f_ncSMO_Image);
            $field_id = 'ncSMO_Image';
            $loaded_file = "<input type='hidden' name='f_ncSMO_Image_old' value='" . htmlspecialchars($f_ncSMO_Image, ENT_QUOTES) . "' />" .
                NETCAT_MODERATION_FILES_UPLOADED . ': ' .
                "<a target='_blank' href='" . nc_get_scheme() . '://' . $_SERVER['HTTP_HOST'] . '/netcat_files/' . $filepath . "'>" . htmlspecialchars($filename, ENT_QUOTES) . '</a> (' . nc_bytes2size($filesize) . ')' .
        ...
        $html .= nc_admin_input(NETCAT_MODERATION_SMO_TITLE, 'smo_title', $sub_env[$nc_core->page->get_field_name('smo_title')], 50);
        if ($mt['smo_title']) {
            $html .= "<div class=\"helper\"> ". NETCAT_MODERATION_SMO_TITLE_HELPER ."</div>";
            $html .= CONTROL_CONTENT_SUBDIVISION_SEO_CURRENT_VALUE.": <b>".htmlspecialchars($mt['smo_title'])."</b><br/>";
        }
        $html .= nc_admin_input(NETCAT_MODERATION_SMO_DESCRIPTION, 'smo_description', $sub_env[$nc_core->page->get_field_name('smo_description')], 50);
        if ($mt['smo_description']) {
            $html .= "<div class=\"helper\"> ". NETCAT_MODERATION_SMO_DESCRIPTION_HELPER ."</div>";
            $html .= CONTROL_CONTENT_SUBDIVISION_SEO_CURRENT_VALUE.": <b>".htmlspecialchars($mt['smo_description'])."</b><br/><br/><br/>";
        }
        // Картинка для SMO
        $html.="<div>
...
function nc_subdivision_form_seo_save() {
    ...
    // метатэги
    $metatags = array('title', 'keywords', 'description', 'smo_title', 'smo_description');
    ...
    $scheme = $nc_core->catalogue->get_scheme_by_id($nc_core->subdivision->get_by_id($sub_id, 'Catalogue_ID'));
    // проверка
    $real_value = $nc_core->page->fetch_page_metatags("{$scheme}://" . $nc_core->DOMAIN_NAME . nc_folder_path($sub_id));
    foreach ($metatags as $v) {
...
function nc_subdivision_show_component($catalogue_id = 0) {
    ...
    $sql = "SELECT `Class_ID` as value, " .
        ...
        "ORDER BY `Class_Group`, `IsAuxiliary`, `Priority`, `Class_ID`";
    ...
    if ($default_class_id > 0) {
        $sql = "SELECT `Class_ID`, `Class_Group`, File_Mode FROM `Class` WHERE `Class_ID` = ".$default_class_id;
        $default_class = (array)$db->get_row($sql, ARRAY_A);
    } else {
        $sql = "SELECT count(Field_ID), TypeOfData_ID, Class_ID FROM `Field` group by Class_ID having Class_ID > 0 and TypeOfData_ID in (1, 3) and count(Field_ID)=2 order by Class_ID limit 1 ";
        $default_class_data = (array)$db->get_row($sql, ARRAY_A);
        $sql = "SELECT `Class_ID`, `Class_Group`, File_Mode FROM `Class` WHERE `Class_ID` = ".$default_class_data['Class_ID'];
        $default_class = (array)$db->get_row($sql, ARRAY_A);
    }
    ...
    if (!empty($classesV4) || !empty($classesV5)) {
        ...
        $html .= "<select size='10' style='width: 30%". ( (empty($classesV5) || empty($classesV4)) ? ";padding: 7px 0" : ""  ) ."' name='Class_Groups' onchange='nc_class_group_change(this);'>";
        if (!empty($classesV5)) {
            $class_groups = array();

            foreach($classesV5 as $class) {
                if (!in_array($class['optgroup'], $class_groups)) {
                    $class_groups[] = $class['optgroup'];
                }
            }
            $html .= "<option class='nc-text-grey' value='0' ".(!$Class_ID ? " selected" : "").">". ((empty($classesV5) || empty($classesV4)) ? "&nbsp;&nbsp;&nbsp;" : "") . NOT_ADD_CLASS . "</option>\n";
            if (!empty($classesV4)) { $html .= "<option disabled='disabled'>" . CONTROL_CLASS . " v5</option>\n"; }
            foreach($class_groups as $class_group) {
                $html .= "<option value='v5-{$class_group}' ". (($default_class['Class_Group'] == $class_group) ? "selected='selected'" : '' ) .">&nbsp;&nbsp;&nbsp;{$class_group}</option>";
            }
        }
        if (!empty($classesV4)) {
            $class_groups = array();

            foreach($classesV4 as $class) {
                if (!in_array($class['optgroup'], $class_groups)) {
                    $class_groups[] = $class['optgroup'];
                }
            }
            if (!empty($classesV5)) { $html .= "<option disabled='disabled'>" . CONTROL_CLASS . " v4</option>\n"; }
            foreach($class_groups as $class_group) {
                $html .= "<option value='v4-{$class_group}'>&nbsp;&nbsp;&nbsp;{$class_group}</option>";
            }
        }
        $html .= "</select>";
        $html .= "&nbsp;<select size='10' style='width: 65%' id='Class_ID' name='Class_ID' onchange='if (this.options[this.selectedIndex].value) {loadClassDescription(this.options[this.selectedIndex].value); loadClassTemplates(this.options[this.selectedIndex].value".($CatalogueID ? ", 0, ".$CatalogueID : ", 0, 0").", 0, \"subdivision\");setInfoblockName(this.options[this.selectedIndex])}'>";
        $html .= "<option value='0'".(!$Class_ID ? " selected" : "").">".NOT_ELSEWHERE_SPECIFIED."</option>\n";
        if (!empty($classesV5)) {
            foreach($classesV5 as $class) {
                if ($class['is_auxiliary']) {
                    $option_class = " class='nc-text-grey' data-class-type='auxiliary' ";
                    $description = $class['description'] . " " . CONTROL_CLASS_AUXILIARY;
                }
                else {
                    $option_class = '';
                    $description = $class['description'];
                }
                $html .= "<option value='{$class['value']}'" . $option_class .
                         " data-nc-classgroup='v5-{$class['optgroup']}'". (($default_class['Class_ID'] == $class['value']) ? "selected='selected'" : '' ) .">$description</option>";
            }
        }
        if (!empty($classesV4)) {
            foreach($classesV4 as $class) {
                ...
                $html .= "<option value='{$class['value']}'" . $option_class .
                         " data-nc-classgroup='v4-{$class['optgroup']}'>$description</option>";
            }
        }
        $html .= "</select>";
	if (!empty($default_class)) {
        ...
        $html .= "<div style='margin:5px 0; _padding:0;'>
                <input name='hide_aux' id='hide_aux' value='0' type='checkbox'>
                <label for='hide_aux'>".CONTROL_CLASS_SHOW_AUX."</label>
                </div>
                <script>
                $(function() {
	          $('input[name=\"Subdivision_Name\"]').on('input', function() {
                     if (!$(this).val()) {
                         $(this).attr('data-changed', 'no');
                     } else {
                         $(this).attr('data-changed', 'yes');
                     }
                  });
                  
                  $('input[name=\"EnglishName\"]').on('input', function() {
                     if (!$(this).val()) {
                         $(this).attr('data-changed', 'no');
                     } else {
                         $(this).attr('data-changed', 'yes');
                  ...
                  function show_aux(){
                    \$nc('SELECT[name=Class_Groups] OPTION').each(function() {
                      var \$this = \$nc(this);
		      if (!\$this.is(':disabled')) {
			 var selected_group = \$this.val();
			 var aux_count = \$('option[data-nc-classgroup=\"'+selected_group+'\"][class=nc-text-grey]').length;
			 var count = \$('option[data-nc-classgroup=\"'+selected_group+'\"]').length;
			 if (aux_count == count) {
			   \$this.hide();
			 }
		       }
                    });
                    if ( !$('SELECT[name=Class_Groups] OPTION:checked').is(':visible') ) {
                      $('SELECT[name=Class_Groups]').val(0);
                      $('SELECT[name=Class_ID]').find('OPTION:contains(\'Не указывать\')').show();
                      $('SELECT[name=Class_ID]').val($('SELECT[name=Class_ID]').find('option:visible:first').val());
                    }
                    $('SELECT[name=Class_ID] OPTION[class=nc-text-grey]').hide();
                    $('SELECT[name=Class_Groups] OPTION[class=nc-text-grey]').show();
                  }
                  show_aux();
                  $('#hide_aux').change(function(){
                    if($('#hide_aux:checked').length == 0) {
                        show_aux();
                    } else {
                        \$nc('SELECT[name=Class_Groups] OPTION').each(function() {
                        var \$this = \$nc(this); 
                        ...
                        var selected_group = \$nc('SELECT[name=Class_Groups] OPTION:checked').val();
                        \$nc('option[data-nc-classgroup=\"'+selected_group+'\"]').each(function() {
                          var \$this = \$nc(this);
                          \$this.show();
                        });
                    }
        ...
        $html .= "<script type='text/javascript'>
	function setInfoblockName(subdivisionItem) {
	    var SubdivisionName = $(\"input[name='Subdivision_Name']\");
	    var EnglishName = $(\"input[name='EnglishName']\");
	    var SubdivisionNameChanged = SubdivisionName.attr('data-changed');
	    var EnglishNameChanged = EnglishName.attr('data-changed');
	    var toRemove = $(subdivisionItem).val()+'. ';
	    var newSubdivisionName = $(subdivisionItem).text().replace(toRemove, '');
	    if (!SubdivisionName.val() || (SubdivisionNameChanged != 'yes')) {
		SubdivisionName.val(newSubdivisionName);
	    }
	    if (!EnglishName.val() || (EnglishNameChanged != 'yes')) {
		EnglishName.val(transliterate(newSubdivisionName, 'yes'));
	    }
        }
        function nc_class_group_change(element) {
            // В Safari нельзя спрятать option внутри select
            var isSafari = navigator.vendor &&
                           navigator.vendor.indexOf('Apple') > -1
                           && navigator.userAgent
                           && !navigator.userAgent.match('CriOS');
            var \$target_select = \$nc('SELECT[name=Class_ID]');

            var \$select = \$nc(element);
            var value = \$select.val();

            // Удаляем ранее созданный разделитель для его точного позиционирования
            \$target_select.find('OPTION[data-line]').remove();
            \$('#loadClassTemplates').empty();

            \$target_select.find('OPTION').each(function() {
                var \$this = \$nc(this);
                var attribute = \$this.attr('data-nc-classgroup');
                var auxiliary  = \$this.attr('data-class-type');

                if (attribute && attribute != value) {
                    if (isSafari) {
                        // Раз спрятать опцию нельзя, выключим ее и отправим в конец списка
                        \$this.attr('disabled', 'disabled');
                        \$target_select.append(\$this);
                    } else {
                        \$this.hide();
                    }
                } else {
                    if (isSafari) {
                        // Включаем опцию и перемещаем в начало списка
                        \$this.removeAttr('disabled');
                        \$target_select.prepend(\$this);
                    } else {
                        \$this.show();
                    }
                }
            ...
            if (isSafari) {
                // Первой должна быть опция 'Не указывать'
                \$target_select.prepend(\$target_select.find('OPTION:contains(\'Не указывать\')'));
                // С помощью разделителя явно покажем, какие опции недоступны для выбора
                \$('<option data-line=\'true\' value=\'0\' disabled=\'disabled\' style=\'color:red;\'>--- Конец доступных для выбора элементов ---</option>')
                    .insertBefore(\$target_select.find('OPTION:disabled:first'));
            }
            if ( $('SELECT[name=Class_Groups] OPTION:checked').val() != 0 ) {
              $('SELECT[name=Class_ID]').find('OPTION:contains(\'Не указывать\')').hide();
              var sel_value = $('SELECT[name=Class_ID]').find('option:visible:first').val();
              $('SELECT[name=Class_ID]').val(sel_value);
              loadClassDescription(sel_value); 
              loadClassTemplates(sel_value, 0, 1, 0, 'subdivision');
            } else {
              $('SELECT[name=Class_ID]').find('OPTION:contains(\'Не указывать\')').show();
              $('SELECT[name=Class_ID]').val($('SELECT[name=Class_ID]').find('option:visible:first').val());
            }
        ...
        var selected_group = \$nc('SELECT[name=Class_ID] OPTION:checked').attr('data-nc-classgroup');
        if (typeof(selected_group) && selected_group) {
            \$nc('SELECT[name=Class_Groups] OPTION').each(function() {
                var \$this = \$nc(this);
                \$this.attr('selected', false);
                if (selected_group == \$this.val()) {
        ...
        nc_class_group_change(\$nc('SELECT[name=Class_Groups]').get(0));
        </script>";
        if ($default_class) {
            $html .= "<script type='text/javascript'>
             $(function() {
                $('option[data-nc-classgroup=\"v5-".$default_class['Class_Group']."\"]').each(function() {
                  var \$this = \$nc(this);
                  \$this.show();
                });
               $('SELECT[name=Class_ID]').find('OPTION:contains(\'Не указывать\')').hide(); 
               $('SELECT[name=Class_Groups]').val('v5-".$default_class['Class_Group']."');
               $('SELECT[name=Class_ID]').val(".$default_class['Class_ID'].");
               loadClassDescription(".$default_class['Class_ID']."); 
               loadClassTemplates(".$default_class['Class_ID'].", 0, 1, 0, 'subdivision');
	       setInfoblockName($('SELECT[name=Class_ID]').find('option:selected'));
             });
            </script>";
        }
    } else {
...
function nc_subdivision_add($input = null) {
    ...
    if ($Class_ID) {
        ...
        $Sub_Class_Name = $db-> get_var('SELECT `Class_Name` FROM Class WHERE `Class_ID` = '. $Class_ID);
