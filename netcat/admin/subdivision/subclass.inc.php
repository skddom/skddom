            ...
            <table border=0 cellpadding=0 cellspacing=0 width=100%><tr>
                        ...
                        <table border=0 cellpadding=0 cellspacing=0 width=100% class='border-bottom'>
                            ...
                            foreach ($Result as $Array) {
                                $hidden_host = $nc_core->catalogue->get_scheme_by_id($Array[12]) . '://' . ($Array[7] ? (strchr($Array[7], ".") ? $Array[7] : $Array[7] . "." . $DOMAIN_NAME) : $DOMAIN_NAME) . $SUB_FOLDER;
    ...
    function  ActionForm($SubClassID, $phase, $type) {
      ...
      WHERE `Class_ID` = '" . ($Class_Template_ID ? $Class_Template_ID : $ClassID ? $ClassID : $selected_value) . "'");
            ...
            $classInfo .= "
                ...
                <select name='subclassType' onchange='if(this.selectedIndex == 1){ \$nc(\"#nc_class_select\").hide(); \$nc(\"#loadClassCustomSettings\").hide(); \$nc(\"#nc_mirror_select\").show(); \$nc(\"#loadClassTemplates\").html(\"\") } else {\$nc(\"#nc_class_select\").show(); \$nc(\"#nc_mirror_select\").hide(); \$nc(\"#loadClassTemplates\").html(\"\")}'>
                <option value='0'>" . CONTROL_CONTENT_SUBCLASS_TYPE_SIMPLE . "</option>
                <option value='1'>" . CONTROL_CONTENT_SUBCLASS_TYPE_MIRROR . "</option>
                </select>";
            
            $classInfo .= nc_sub_class_get_classes($Array, $classesV4, $classesV5);
        ...
        if ($type == 2) {
            ...
            $Array = $db->get_row(
                "SELECT c.`Class_Name`, c.`System_Table_ID`, c.`CustomSettingsTemplate`, 
                        s.* 
                   FROM `Sub_Class` AS s
...
function ActionSubClassCompleted($type) {
    ...
    $params = array('Priority', 'Checked', 'SubClassName', 'EnglishName', 'Class_Template_ID', 'Class_ID',
...
function nc_sub_class_get_classes($Array, $classesV4, $classesV5) {
    $nc_core = nc_Core::get_object();
    $db = $nc_core->db;
    ...
    if (!empty($classesV4) || !empty($classesV5)) {
        ...
        $html .= "&nbsp;<select size='10' style='width: 65%' id='Class_ID' name='Class_ID' onchange='if (this.options[this.selectedIndex].value) {loadClassDescription(this.options[this.selectedIndex].value); loadClassTemplates(this.options[this.selectedIndex].value".($CatalogueID ? ", 0, ".$CatalogueID : ", 0, 0").");setInfoblockName(this.options[this.selectedIndex])}'>";
                if (!empty($classesV5)) {
            foreach($classesV5 as $class) {
                if ($class['is_auxiliary']) {
                    $option_class = " class='nc-text-grey'";
        ...
        $html .= "</select>";
                ...
                <script>
                $(function() {
		  $(\"input[name='SubClassName']\").on('input', function() {
		     if (!$(this).val()) {
			 ...
			 $(this).attr('data-changed', 'yes');
		      ...
		      if (!\$this.is(':disabled')) {
                    ...
                    if ( !$('SELECT[name=Class_Groups] OPTION:checked').is(':visible') ) {
                      var first_group = $('SELECT[name=Class_Groups]').find('option:visible:first').val();
                      $('SELECT[name=Class_Groups]').val(first_group);
                      \$nc('option[data-nc-classgroup=\"'+first_group+'\"]').each(function() {
                          var \$this = \$nc(this);
                          \$this.show();
                        });
        ...
        $html .= "<script type='text/javascript'>if (" . ($Class_ID + 0) . ") {loadClassTemplates(" . ($Class_ID ? $Class_ID : 0) . ($Class_Template_ID ? ", ".$Class_Template_ID : ", 0") . ($CatalogueID ? ", " . $CatalogueID : "") . ");}</script>";
        $html .= "<script type='text/javascript'>
	function setInfoblockName(infoblockItem) {
	    var SubClassName = $(\"input[name='SubClassName']\");
	    var EnglishName = $(\"input[name='EnglishName']\");
	    var SubClassNameChanged = SubClassName.attr('data-changed');
	    var EnglishNameChanged = EnglishName.attr('data-changed');
	    var toRemove = $(infoblockItem).val()+'. ';
	    var newSubClassName = $(infoblockItem).text().replace(toRemove, '');
	    if (!SubClassName.val() || (SubClassNameChanged != 'yes')) {
		SubClassName.val(newSubClassName);
	    }
	    if (!EnglishName.val() || (EnglishNameChanged != 'yes')) {
		EnglishName.val(transliterate(newSubClassName, 'yes'));
	    }
        }
        function nc_class_group_change(element) {
            ...
            if ( $('SELECT[name=Class_Groups] OPTION:checked').val() != 0 ) {
              ...
              if(sel_value) {
                ...
                loadClassTemplates(sel_value, 0, 1, 0);
		loadClassCustomSettings(sel_value);
              }
            } 
        }
        ...
        if ($default_class) {
            $html .= "<script type='text/javascript'>
             $(function() {
               ...
               loadClassTemplates(" . $default_class['Class_ID'].", 0, 1, 0);
	       loadClassCustomSettings(" . $default_class['Class_ID'] . ");
             });
            </script>";
        }
    } else {
...
function nc_sub_class_template_select($template_type, $component_id, $current_component_template_id) {
    ...
    $result .=
        "<script>
            $('#nc_button_$select_name').click(function() {
