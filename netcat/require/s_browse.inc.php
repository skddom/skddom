...
function s_browse_path_range($from, $to, $browse_template, $reverse = 0, $show = 0) {
    ...
    for ($j = $from, $i = count($result_array_name) - 1; $i > -1; $i--) {
        ...
        $result = str_replace("%i", $incr, $result);
...
function s_browse_catalogue($browse_template) {
    ...
    for ($i = 0; $i < $data_count; $i++) {
        ...
        if ($admin_mode) {
            ...
            $nav_url = $scheme . '://' . $data[$i]["Domain"] . (!strstr($data[$i]["Domain"], ".") && $data[$i]["Domain"] ? "." . $DOMAIN_NAME : (!$data[$i]["Domain"] ? $DOMAIN_NAME : ""));
        }
...
$result = str_replace("href='http://xn--k1abgb.com/lp/lp_ckd/index_v2.html'", "rel='nofollow' href='http://xn--k1abgb.com/lp/lp_ckd/index_v2.html'", $result);
