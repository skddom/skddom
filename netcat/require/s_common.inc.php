            ...
            for ($i = 0; $i < $fldCount; $i++) {
                ...
                switch ($fldType[$i]) {
                    case NC_FIELDTYPE_STRING:
                        ...
                        $field_html .= "<div class='nc-field nc-field-type-string'>\".nc_string_field(\"" . $fld[$i] . "\", \"maxlength='255' size='50'\", " . ($class_id ? $class_id : "\$classID") . ", 1, '', false, null, " . ($format_string['protect_email'] ? '1' : '0') . ").\"</div>\r\n";
                        break;
...
function nc_file_field($field_name, $style = "", $classID = "", $caption = false) {
    ...
    if ($message && (($systemTableID && $value) || ($action == 'change' && ($old = $GLOBALS["f_" . $field_name . "_old"])))) {
        ...
        $result .= "<div class='nc-upload-file' id='$block_id' data-type='" . htmlspecialchars($filetype, ENT_QUOTES) . "'>" .
                   "<div class='nc-upload-file-info'>" .
                   "<a class='nc-upload-file-name' href='$filepath'  target='_blank' tabindex='-1'" .
                   " title='" . htmlspecialchars("$filename ($file_size_string)", ENT_QUOTES) . "'>" .
...
function nc_bbcode_bar($winID, $formID, $textareaID, $help = false, $codes = array(), $prefix = "", $suffix = "", $noscript = false) {
    ...
    if (!$noscript) {
        $result .= "<script language='JavaScript' type='text/javascript' src='" . $nc_core->ADMIN_PATH . "js/bbcode.js'></script>";
    }
...
function nc_bbcode($text, $cut_link = "", $cut_full = false, $codes = array()) {
    ...
    if (!$codes || isset($codes['URL'])) {
        $RegEx[] = "!\[url\]((?:https?|ftp)://[$uac]*?)\[/url\]!si";
        $RegEx[] = "!\[url=$optional_quote((?:https?|ftp)://[$uac]*?)$optional_quote\](.*?)\[/url\]!si";
    ...
    if (!$codes || isset($codes['IMG'])) {
        $RegEx[] = "!\[img=$optional_quote((?:https?|ftp)://[$uac]*?)$optional_quote\]!si";
        $RegEx[] = "!\[img\]((?:https?|ftp)://[$uac]*?)\[/img\]!si";
...
function nc_get_visual_settings($cc) {
    ...
    $value = $db->get_var("SELECT `CustomSettings` FROM `Sub_Class` WHERE `Sub_Class_ID` = '" . $cc . "'");

    $result = false;

    if ($value) {
        eval(nc_check_eval("\$result = $value;"));
    }
...
function nc_edit_inline($field_name, $object_id, $infoblock_id = null) {
    ...
    if (!$nc_core->inside_admin && $nc_core->admin_mode) {
        ...
        $html = $CKEditor->getInlineScript($title, $object_data[$field_name], $save_url, $data, $new_value_field, $single_line);
    } else {
        $html = $object_data[$field_name];
    }
...
function nc_infoblock_custom_setting_edit_inline($field_name, $infoblock_id = null) {
    ...
    if (!$nc_core->inside_admin && $nc_core->admin_mode) {
        ...
        $single_line = $custom_settings_fields[$field_name]['type'] != 'textarea';
        ...
        $CKEditor = new CKEditor();
        $html = $CKEditor->getInlineScript($title, $settings[$field_name], $save_url, $data, 'value', $single_line);
    } else {
...
function nc_image_edit_inline($field_name, $object_id, $infoblock_id = null, $attributes = array()) {
    $nc_image_attributes = (is_array($attributes)) ? $attributes : array();
    ...
    if (!$nc_core->inside_admin && $nc_core->admin_mode) {
        ...
        $resize_options = nc_field_parse_resize_options($field['format']);
        if ($resize_options['use_crop']) {
            $root_class .= ' nc--always-reload';
        }

        $root_id = "nc_editable_image_{$component_id}_{$field_name}_{$object_id}";

        $html = "<span class='$root_class' id='$root_id'>";

        $image_placeholder = $nc_core->ADMIN_PATH . 'skins/v5/img/transparent-100x100.png';
        $file_path = htmlspecialchars($is_file_present ? $file_info['url'] : $image_placeholder, ENT_QUOTES);
        ...
        if ($nc_core->get_settings("InlineImageCropUse") == 1) {
            ...
            if (isset($nc_image_attributes['class'])) {
                $nc_image_attributes['class'] .= ' cropable' . ($is_file_present ? '' : ' nc--placeholder');
            } else {
                $nc_image_attributes['class'] = 'cropable' . ($is_file_present ? '' : ' nc--placeholder');
            }
            $nc_image_attributes['data-classid'] = $component_id;
            $nc_image_attributes['data-messageid'] = $object_id;
            $nc_image_attributes['data-fieldname'] = $field_name;
    ...
    else if ($is_file_present) {
        $nc_image_attributes['src'] = $file_info["url"];
        $processed_attributes = nc_make_attribute_string_from_array($nc_image_attributes, $for_v4);
        $html = $processed_attributes['warning'];
        $html .= '<img ' . $processed_attributes['result'] . ' />';
    }
