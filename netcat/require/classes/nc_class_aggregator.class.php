        ...
        foreach ($settings as $class_id => $setting) {
            ...
            foreach ($setting as $component_field_name => $aggregator_field_name) {
                ...
                $prepare_setting[] = "(IF (@type := (SELECT (IF((POSITION('fs3' IN Format) > 0), 3,
                                                    ...
                                                    FROM Field WHERE Class_ID = $class_id AND Field_Name = '$component_field_name' AND TypeOfData_ID = 6),
                                            (IF(@id := (SELECT Field_ID FROM Field WHERE Class_ID = $class_id AND Field_Name = '$component_field_name' AND TypeOfData_ID = 6),
