...
function GetCatalogueID ($host) {
  ...
  $res = $db->get_var("SELECT `Catalogue_ID` FROM `Catalogue` WHERE IF(LOCATE('.',Domain),Domain,IF(Domain<>'',CONCAT(Domain,'.".$db->escape($DOMAIN_NAME)."'),'".$db->escape($DOMAIN_NAME)."'))='".$db->escape($host)."'");
  ...
  if ($db->num_rows) {
    ...
    $res = $db->get_results("SELECT `Catalogue_ID`, `Mirrors` FROM `Catalogue` ORDER BY `Catalogue_ID`", ARRAY_N);
    if ( empty($res) ) return 0;
    foreach ($res as $row) {
      list($CatalogueID, $Mirrors) = $row;
      $DomainArray = explode("\n",$Mirrors);
      for ($i=0; $i < count($DomainArray); $i++) {
        if ( trim($DomainArray[$i])==$host ) {
          return ($CatalogueID);
        }
      }
    }
